-- ============================================================================
-- RESTROZ DEV ENVIRONMENT — AUTH & USER RECONCILIATION PATCH
-- Environment: DEVELOPMENT (restroz-dev ONLY)
-- Target Database: restroz-dev Supabase Project
-- ============================================================================
-- Purpose:
-- Fixes "Database error querying schema" on login by completely reconciling and
-- provisioning the 5 DEV accounts in auth.users, auth.identities, profiles,
-- restaurant_members, and restaurant_member_permissions with verified credentials:
--
-- Accounts (Password: Password123!):
-- 1. rdsa@yopmail.com  (Super Admin)
-- 2. ppad@yopmail.com  (Panch Phoron Admin)
-- 3. ppst@yopmail.com  (Panch Phoron Cashier / Staff)
-- 4. ppkt@yopmail.com  (Panch Phoron Kitchen Staff)
-- 5. ppcu@yopmail.com  (Panch Phoron Customer)
--
-- Safety Guarantees:
-- 1. Strictly non-destructive to Panch Phoron business data (orders, menu, settings).
-- 2. Fully compatible with current Supabase GoTrue Auth schema.
-- 3. Idempotent — safe to execute multiple times.
-- ============================================================================

-- ----------------------------------------------------------------------------
-- 0. SCHEMA RECONCILIATION (Ensure required columns exist across tables)
-- ----------------------------------------------------------------------------
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS phone TEXT;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS avatar_url TEXT;

ALTER TABLE public.restaurant_members ADD COLUMN IF NOT EXISTS is_active BOOLEAN DEFAULT TRUE;

-- ----------------------------------------------------------------------------
-- 1. PROVISION AUTH USERS, IDENTITIES, PROFILES & MEMBERSHIPS
-- ----------------------------------------------------------------------------
DO $$
DECLARE
    v_pw_hash TEXT;
    ...
    v_mem_id UUID;
BEGIN
    RAISE NOTICE 'Starting RestroZ DEV Auth & User Reconciliation Patch...';
    ...
    FOR v_u IN 
        SELECT * FROM (VALUES
            ('a0000000-0000-0000-0000-000000000001'::uuid, 'rdsa@yopmail.com', 'Ratnadeep Dey', 'SUPER_ADMIN', '+91 99999 00001', NULL::uuid),
            ...
            ('a0000000-0000-0000-0000-000000000005'::uuid, 'ppcu@yopmail.com', 'Panch Phoron Customer', 'CUSTOMER', '+91 98765 43213', NULL::uuid)
    ...
    LOOP
        ...
        IF v_existing_id IS NOT NULL THEN
            ...
            -- Update existing auth.users record with verified GoTrue credentials
            UPDATE auth.users
            ...
            RAISE NOTICE 'Updated existing auth.users for % (%)', v_u.email, v_user_id;
        ELSE
            ...
            RAISE NOTICE 'Created new auth.users for % (%)', v_u.email, v_user_id;
        END IF;

        -- Clean up existing sessions & identities to prevent token/identity mismatch
        DELETE FROM auth.identities 
        WHERE user_id = v_user_id 
           OR (provider = 'email' AND provider_id = v_user_id::text)
           OR (provider = 'email' AND provider_id = v_u.email);

        -- Insert clean matching auth.identities record
    ...
    RAISE NOTICE '✅ All 5 DEV users successfully reconciled and provisioned with password "Password123!".';
END $$;
