-- ==============================================================================
-- MIGRATION: 20260916000001_qr_table_occupancy_and_sup_orders.sql
-- DESCRIPTION: Add is_supplementary column, partial active orders index, update trigger,
--              and update create_guest_qr_order RPC with race-safe table occupancy check.
-- ==============================================================================

-- 1. Add is_supplementary column to orders
ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS is_supplementary BOOLEAN NOT NULL DEFAULT FALSE;

-- 2. Create high-efficiency partial index for active table occupancy lookup
...
-- 3. Update validate_order_tenant_and_table trigger to handle is_supplementary & table occupancy
...
BEGIN
    ...
    IF (NEW.table_id IS NOT NULL) THEN
        ...
        IF (TG_OP = 'INSERT') THEN
            -- If this is a Customer QR order OR a customer/anon direct insert, block if table has ANY active unpaid order
            IF (NEW.order_source = 'CUSTOMER_QR' OR NOT is_staff_member) THEN
...
-- 4. Update create_guest_qr_order RPC with row-level table lock and occupancy protection
