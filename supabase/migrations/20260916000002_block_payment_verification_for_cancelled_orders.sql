-- ==============================================================================
-- RESTROZ MIGRATION: BLOCK PAYMENT VERIFICATION FOR CANCELLED ORDERS
-- File: 20260916000002_block_payment_verification_for_cancelled_orders.sql
-- Description:
--   1. Rejects payment verification for orders with status = 'cancelled'.
--   2. Preserves already-recorded payment history for previously paid orders.
--   3. Maintains role-based authorization, tenant isolation, and payment method mapping.
-- ==============================================================================
...
BEGIN
    ...
    -- 5. Cancelled Order Guard: Cannot verify payment for a cancelled order
    ...
    -- Map order payment method to payments table allowed domain ('cash', 'card', 'upi', 'room', 'split')
    v_payment_method := CASE LOWER(COALESCE(v_order.payment_method, 'online'))
    ...
    -- 6. Idempotent Check: If already paid, return existing state
    ...
    -- 8. Insert Payment Record (payments table schema: id, order_id, payment_method, amount, status, reference_number, notes, created_at)
